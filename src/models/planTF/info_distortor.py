...
class InfoDistortor:
    ...
    * minimize dist(perturbed_trajectory, ground_truth_trajectory)
    ...
    def augment(
        ...
        ori_device = data_batch["current_state"].device
        data_batch = NuplanFeature(data_batch).to_numpy().unpack()

        distorted_list = []

        for nf in data_batch:
            ...
            if not self.safety_check(
                ...
                distorted_list.append(nf.to_feature_tensor())
                continue
            ...
            if self._normalize:
                distorted_list.append(NuplanFeature.normalize(data).to_feature_tensor())

        distorted = NuplanFeature.collate(distorted_list).to_device(ori_device).data
