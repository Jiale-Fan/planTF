...
class TFTrajectoryDecoder(nn.Module):
    ...
    def forward(self, agent_emb, map_emb, agent_mask, map_mask):
        ...
        mode_params_emb_2 = self.mode_map_attn(query=mode_params_emb, key=map_emb, value=map_emb,
                                                key_padding_mask=map_mask
                                                )[0] + mode_params_emb
        mode_probs_3 = self.prob_predictor(mode_params_emb_2)
        mode_probs_4 = mode_probs_3.squeeze(-1)
        mode_probs = F.softmax(mode_probs_4, dim=-1)
...
class OutputModel(nn.Module):
    ...
    def forward(self, agent_latent_state):
        ...
        pred_obs = pred_obs.reshape(B, modes, agents, self.future_steps, -1)
        x_mean = pred_obs[..., 0]
        y_mean = pred_obs[..., 1]
        ...
        if self.predict_yaw:
            ...
            yaws_cos = pred_obs[..., 5]
            yaws_sin = pred_obs[..., 6]
            return torch.stack([x_mean, y_mean, x_sigma, y_sigma, rho, yaws_cos, yaws_sin], dim=-1)
        else:
